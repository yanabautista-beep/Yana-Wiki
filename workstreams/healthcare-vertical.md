---
summary: Healthcare at Home vertical — Lab Tests, IV Therapy, Doctor Consultation, Pet Healthcare, Glutathione, Cancer Screening, Peptides.
status: Active
last_updated: 23.04.2026
---

# Healthcare Vertical

## Current Status

Healthcare is the primary growth vertical across paid and CRM. Multiple sub-verticals are active: Lab Tests at Home, IV Therapy, Doctor Consultation at Home, Pet Healthcare, Glutathione IV, Cancer Screening, Mental Health. Concept campaigns are in progress. Funnel drop-off analysis underway for Pet Healthcare.

32% WoW decline (flagged 17.04) — attributed to IV Therapy (one-off) and lab test (slightly soft due to credits + non-discounted bookings). Budget reallocation done 16.04. Lab Test WA campaign planned for Sunday 20.04 — status unconfirmed.

## Sub-Verticals & Pricing

| Service | AUH | DXB / SHJ / AJM |
|---|---|---|
| Doctor Consultation at Home (Physical Screening) | AED X | AED 249 |
| Cancer Screening Lab Test | AED 499 | AED 449 |
| Lab Test at AED 49 | — | AED 49 (Vitamin D, Diabetic Profile) |
| Glutathione IV Therapy | — | TBD (pending brand approval) |
| Pet Healthcare | — | Active |
| Pet Relocation | AED 3,749 | AED 3,749 |
| Mental Health (Free) | — | Active (Google Search; Meta creatives pending) |
| Skin, Hair & Nails IV Drip | — | Live as of 20.04.2026 (new attribute, trackable) |

## Key Initiatives

- **HC Concept Ads** — new creative concepts being developed; add creatives and take live [OPEN 6d]
- **Pet Healthcare Thank You Pop Up** — **RESOLVED 20.04.2026** — Sabhyata fixed logic; Nitish confirmed working
- **Pet Healthcare Push Notification** — [OPEN 4d]
- **Pet Healthcare CRM Ecom Tile** — [OPEN 4d]
- **Pet Healthcare Funnel Drop Off** — analysis in progress [OPEN 6d]
- **Glutathione IV Therapy Google Search** — campaign prepared, pending brand ad copy approval
- **Lab Test Survey Like Creative Campaign** — alignment pending [OPEN 6d]
- **Diabetic Profile Meta campaign** — HbA1C at AED 49, under review [OPEN 4d]
- **Mental Health Campaign** — Google Search not performing; no further optimization. Meta campaign pending creatives (end of week / early next week).
- **Pet Grooming Revamp** — funnel and creatives updated by Rabin, live 17.04.2026
- **IV Therapy funnel update** — Hair Strengthening Drip inactivated; Skin, Hair & Nails IV Drip now live as new attribute (20.04.2026, Rabin)
- **Peptide launch** — **LIVE 23.04.2026** (one day before 24.04 deadline). Rabin M confirmed in #healthtasks. Nitish cross-checking pricing and key nutrients per brief.
- **HC Concept Test results** — V1 wins (5-day results): V1 Meta CPI $130.70 / Adjust $94.40 vs V2 $309.68 / $212.91; Version C best delivery (73%). Bilge recommends call to finalise structure with Ameya, then share with brand team (20.04.2026)

## Pet Grooming Sunday Metrics (20.04.2026)
- Total Bookings: 233
- CX Cancellations: 27
- Bookings Completed: 206
- Rating: 4.83 (82 ratings: 66×5★, 9×4★, 2×3★, 1×1★)

## CRM Cross-Dependency

Sabhyata owns CRM tasks for Healthcare. Pending tasks include:
- CRM Ecom Tile for Pet Healthcare
- Funnel drop-off content and WhatsApp
- Lab Test WA Sunday send (status unconfirmed)

## Decisions Made

- Decided to launch Glutathione IV Therapy before Peptide vertical due to higher search volume and better deal terms (13.04.2026)
- Paused Arooj ad, replaced with new ads on HC App Install campaign
- AB test for instant vs non-instant Healthcare ads completed — results used to inform future creative structure
- Reverted from AED 50 Off to HC10 deal based on cohort analysis (lower CPA, comparable retention)
- Mental health Google Search: stop optimizing; wait for Meta creatives before scaling (20.04.2026)

## Blockers

- Brand team approval pending on Glutathione ad copies
- Mental health Meta creatives expected end of week (Apr 2026)
- Pet Healthcare creative brief submitted to BD; prioritization needed
- HC Concept Test structure — call with Bilge + Ameya needed to finalise before HC Concept Ads go live
- Urine Culture & Sensitivity Test naming — 0 search volume vs clinical need debate; "Urine analysis & culture test" proposed by Sandeep (23.04.2026)
- April conversion drop — PG and PH conversions declining in April despite session spikes; Nitish correlating with launch of search campaigns; Yana + Bilge to investigate (23.04.2026)

## Open Items

| Item | Owner | Status | Days Open |
|---|---|---|---|
| Pet Healthcare Push Notification | Yana | Open | 4 |
| Pet Healthcare CRM Ecom Tile | Sabhyata | Open | 4 |
| Funnel Drop Off — Pet Healthcare (Content & WA) | Yana / Sabhyata | Open | 6 |
| HC Concept Ads — add creatives and take live | Yana | Open | 6 |
| Healthcare concept ideas brief and alignment | Yana | Open | 6 |
| Lab Test Survey Like Creative Campaign | Yana | Open | 6 |
| Diabetic Profile Meta campaign (HbA1C at AED 49) | Yana | Open | 4 |
| Glutathione Google Search — take live | Yana | Pending brand approval | — |
| KSA Conversion Lift Results | Yana | Open | 4 |
| Complete Analysis on all KSA campaigns | Yana | Open | 6 |
| Lab Test WA Sunday — confirm status | Sabhyata | Unknown | 0 |
| Free Mental Health bookings report | Sandeep / Nitish | Requested | 0 |
| Meta creative placement fix (pet HC pop up) | Yana | Open | 0 |
| HC Concept Test — schedule call with Bilge + Ameya | Yana | Open | 3 |
| Flyers — distribution plan + slot tracking | Yana | In print; plan unclear | 3 |
| HC Concept Ads — review Asana tags from Pallavi | Yana | Pending | 3 |
| April conversion drop (PG/PH) investigation | Yana / Bilge | NEW | 0 |
| Urine Culture & Sensitivity Test naming decision | Yana / Sandeep | Debate ongoing | 0 |
| AED 10 Zonal HC booking numbers — share with Rabiah | Yana | NEW | 0 |

## Backlinks

- [[workstreams/paid-media]] — Paid campaigns for healthcare
- [[workstreams/crm-email]] — Sabhyata owns CRM tasks
- [[workstreams/ksa-expansion]] — KSA healthcare campaigns
- [[experiments/iv-therapy-meta-ab-test]] — IV Therapy A/B test
- [[experiments/meta-ab-test-hc]] — HC concept test
- [[people/sabhyata]] — CRM owner for Healthcare
